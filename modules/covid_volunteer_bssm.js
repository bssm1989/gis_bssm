(function(){
	...
	LoadMap(function(map){
		...
		var _types=[
			{label:'รายได้',value:'รายได้',icon:'icons/retirement_home.png'},
		...
		$.ajax({
								...
								type:'POST',
								url:'database_tions.php',
								data:{where:"type='รายได้'"},
								success:function(res){
									
									covid_locations[0] = jsonParse(res);
									if(covid_locations[0]){
										$.each(covid_locations[0],function(){
											if(this.lat && this.lng){
												addMarker_income(this);
		...
		function addMarker_income(o){
			var $$ = this;
			if(!o.lat || !o.lng){
				return false;
			}
			var id = 'covid_locations_'+o.HC+'|'+o.id;
			...
			var boxImage = 'data:image/gif;base64,iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAQAAABuBnYAAAAAEUlEQVR42mP8/58BBTAOjAAArngP+UcN9BIAAAAASUVORK5CYII=';
			
			
			var colorLevel=1;
			var r,g,b;
			if(o.income<3001){
				colorLevel=Math.round(o.income/3000*100)/100;
				r=1;
				g=colorLevel;
				b=0;
				
			}else if(o.income<5418)
			{
				colorLevel=Math.round((o.income-3000)/(5418-3000)*100)/100;
				g=1;
				r=1-colorLevel;
				b=0;
			...
			/* var lat_last=Math.floor(Math.random()*(999-100+1)+100)/10000;
				var lng_last=Math.floor(Math.random()*(999-100+1)+100)/10000;
				var lat=(122.1+lat_last)*-1;
				var lng=(46.1+lng_last); */
			var p={
            ...
            console.log(error);
							...
							var d = _this.value;
							if(d.type){
								s = s+'ประเภท : '+o.income+'<br>';
							}
							if(d.lat){
								s=s+d.PERSON+'<br>';
								
								s=s+'ที่อยู่:บ้านเลขที่  '+d.MBNO+' หมูที่ '+d.MB+' ต. '+d.tambon_name_thai+' อ. '+d.district_name_thai+' จ.'+d.province_name_thai+'<br>';
								s=s+'รายได้ :'+o.income+'<br>';
								s=s+'ละติจูด :'+o.lat+'<br>';
								s=s+'ลองจิจูด :'+o.lng+'<br>';
							}
							if(d.description){
				...
				billboard:{
					 image : boxImage,
					color : new Cesium.Color(r, g, b, 0.5),
  ...
  //map.terrainProvider = Cesium.createWorldTerrain();
