...
$$.getGistdaAdmin =function(LL,cb){
	var query = {geometry:LL.lng+','+LL.lat,geometryType:"esriGeometryPoint",inSR:4326,outSR:4326,spatialRel:"esriSpatialRelWithin",outFields:"*",returnGeometry:false,f:"pjson"}
	 $.ajax({
		type:'POST',
		url:'https://gistdaportal.gistda.or.th/arcgis/rest/services/app/admin_poly/MapServer/99/query',
		crossDomain:true,
		data:$.param(query),
...
$$.getAdmin =function(LL,cb){
	var query = {geometry:LL.lng+','+LL.lat,geometryType:"esriGeometryPoint",inSR:4326,outSR:4326,spatialRel:"esriSpatialRelWithin",outFields:"*",returnGeometry:false,f:"pjson",token:''}
	 ...
	 $.ajax({
		type:'POST',
		url:'https://gisportal.fisheries.go.th/arcgis/rest/services/Hosted/%E0%B8%82%E0%B8%AD%E0%B8%9A%E0%B9%80%E0%B8%82%E0%B8%95%E0%B8%95%E0%B8%B3%E0%B8%9A%E0%B8%A5WGS84/FeatureServer/99/query',
...
$$.getLatLngAdminID = function(opts,cb){
	...
	if(where){
		...
		$$.gettingLatLng = true;
		 $.ajax({
			type: 'POST',
			url: server+L+'/FeatureServer/99/query',
			crossDomain: true,
			data:query,
			success: function(data){
				$$.gettingLatLng = false;
				...
				if(data.features){
					...
					//$$.getLatLngAdminID(opts,cb); ป้องกันวนลูป
